...
const inActiveclassName = "text-base block py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-white dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700";
  return (
    ...
    <button onClick={() => setDarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" data-toggle-dark="light" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
      ...
      <div id="nav-links" className= {`${hidden ? "hidden" : ""} justify-between items-center hidden w-full md:flex md:w-auto md:order-1" id="navbar-search`}>
        ...
        <ul classNameName="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
          <li>
            <NavLink to="/home" classNameName={({isActive}) => isActive ? activeclassName : inActiveclassName } end> Home</NavLink>
          </li>
          <li>
            <NavLink to="/movies/popular" classNameName={({isActiveclassName}) => isActiveclassName ? inActiveclassName : inActiveclassName }> Popular</NavLink>
          </li>
          <li>
            <NavLink to="/movies/top" classNameName={({isActiveclassName}) => isActiveclassName ? inActiveclassName : inActiveclassName }>Top Rated</NavLink>
          </li>
          <li>
            <NavLink to="/movies/upcoming" classNameName={({isActiveclassName}) => isActiveclassName ? inActiveclassName : inActiveclassName }>Upcoming</NavLink>
          </li>
          <li>
            <NavLink to="/" classNameName={({isActiveclassName}) => isActiveclassName ? inActiveclassName : inActiveclassName }></NavLink>
          </li>
        </ul>
      </div>
    </div>
